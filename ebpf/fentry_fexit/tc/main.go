...
func handlePerfEvent(ctx context.Context, events *ebpf.Map) {
	...
	for {
		...
		switch ev.ProbeType {
		case 1:
			log.Printf("Tracing packet: %s -> %s (fentry)", netip.AddrFrom4(ev.Saddr), netip.AddrFrom4(ev.Daddr))
		case 2:
			log.Printf("Tracing packet: %s -> %s (fexit: %s)", netip.AddrFrom4(ev.Saddr), netip.AddrFrom4(ev.Daddr), ev.Verdict)
		}
